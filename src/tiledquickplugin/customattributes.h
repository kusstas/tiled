#pragma once

namespace TiledQuick
{
constexpr auto START_CALLBACK_NAME = "onStart";
constexpr auto DESTROY_CALLBACK_NAME = "onDestroy";
constexpr auto TIMEOUT_CALLBACK_NAME = "onTimeout";
constexpr auto COLLIDE_ENTER_CALLBACK_NAME = "onCollideEnter";
constexpr auto COLLIDE_EXIT_CALLBACK_NAME = "onCollideExit";
constexpr auto PRESS_CALLBACK_NAME = "onPress";
constexpr auto CLICK_CALLBACK_NAME = "onClick";
constexpr auto RELEASE_CALLBACK_NAME = "onRelease";
constexpr auto DRAW_DEBUG_NAME = "drawDebug";
constexpr auto PRESS_BY_SHAPE_NAME = "pressByShape";
constexpr auto CLICKABLE_NAME = "clickable";
constexpr auto COLLIDING_ENABLED_NAME = "collidingEnabled";
}
